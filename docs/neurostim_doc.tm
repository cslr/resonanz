...
<\body>
  ...
  <\itemize-dot>
    ...
    software be more general than using measurements from a single device
    ...
    information about the current picture too when chosing the next picture
    and calculate <math|p<rsub|j*i><around*|(|\<b-t\><rsub|0>|)>> where
    <math|j> is the next stimulation element and <math|i> is the current
    stimulation element. Unfortunately, with large number of different
    stimulation elements (pictures) this grows as
    <math|O<around*|(|N<rsup|2>|)>> and the number of combinations to
    estimate prediction function for each picture pair becomes too large
    ...
    better model could be a differential equation model like linear one:
  ...
  of the product or have a picture of a ad and then the software estimates
  how user is likely to react to it. What colors cause marketing message to
  stay in customer's head the longest etc. The idea is to calculate feature
  vectors <math|\<b-f\>> from pictures and measure users response. The learnt
  model is then part of the program and can be used to any picture to predict
  how much the customer will react to it. The user can then try different
  product/ads designs/shapes/colors and select the one with the best
  response.
